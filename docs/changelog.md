...
- **Fixed** — "Overdue" and "Expired" are derived states now: an invoice still owed after its due date reads Overdue in lists, filters, the dashboard and the client page (the tiles counted statuses nothing sets, so they read 0); the quote expiry check targeted a post type that does not exist, so quotes never expired — open quotes past their date are expired, and an expired, accepted or declined quote cannot be accepted or declined again.
- **Changed** — Stock emails rewritten as plain business correspondence with a "View and pay" button and an `{{amount_due}}` placeholder; sites still on the old stock text are moved over once. Receipts go out for every payment (instalments included) with the right amount, method, date and balance remaining — they used to print `{{payment_40}}` and only after full payment.
- **Changed** — Money is formatted one way everywhere (payment panel, statements, dashboard, reports); a separate **Line total** label for the item table; statements name payments by method.
- **Fixed** — **Trashing marked a paid invoice cancelled and restoring made it Available**; an invoice comes back from the trash with the status it had. The Draft tab lists drafts (it listed everything); "Move to draft" is refused for paid or part-paid invoices and the bulk actions match the row actions. Quote and invoice models gained real `getMeta()` / `setMeta()` methods — the old calls silently did nothing, which several Pro addons had relied on.
- **Fixed** — **`POST /easy-invoice/v1/invoices` now creates a real invoice**: it used to leave the status blank, skip the client record and fire none of the creation hooks. It takes `client_id`, `customer_name`/`customer_email`, `currency`, `terms` and an optional `draft`/`available` status; invoice responses carry `totals.paid`, `totals.credited` and `totals.due`. The [API reference](./api-reference) now documents the REST routes (it said there were none).
- **Fixed** — **Removing a payment left the invoice "Paid"**: trashing, restoring or deleting a payment on the Payments screen now re-checks the invoice against its remaining payments and credit notes. A quote's acceptance and decline details (date, who, reason) are stored, and a client reopening a closed quote is told what happened. Adding a client with a taken email says why. The Reports overview's status chart uses the same derived statuses as the list and honours the date range.
- **Added** — An uninstall routine with an opt-in **Remove all data when the plugin is deleted** setting (Settings → Advanced); off by default. About 140 admin strings that were plain text are now translatable, and Pro ships a `.pot` file.
...
- **Fixed** — **The basic payment reminder never sent either** (it read the client address with a method that does not see it, and chased statuses no sent invoice holds); it now sends, with the secure link and the amount still owed, in the branded layout. Quote watermarks follow the same rules as invoice ones (no stamp on a sent quote).
...
- **Added** — Template Builder **Data tab**: render the canvas with one of your real invoices or quotes instead of sample data. Delete on the Webhooks, Time Tracking, Team Roles and White-Label pages asks for confirmation again (the dialog never loaded).
- **Fixed** — **Client Portal actions refused the invoice's own client** (the ownership check read a meta key nothing writes); view, download, pay and receipt links work. Quote pages get their watermark for browser PDFs. The payments CSV has its Client column. Legacy recurring code and admin templates nothing loaded are removed.
- **Fixed** — **[Email Enhancements](./addons/email-enhancements) had no settings anywhere** (its fields were registered on a tab the plugin never rendered). Rebuilt with its own page: a Reply-To for all emails or per kind, and a custom email layout with a one-click test. White-Label's email and PDF footers work now (the filters were never applied); reminders go out with the From/Reply-To from Settings → Email instead of "WordPress".
- **Fixed** — Time Tracking and Expense Tracking drafts go through the invoice repository (business name, payment terms, numbering, hooks); clients are named properly; trashing or deleting the invoice frees the entries again. Item Library prices use the site's currency in the builder picker.
- **Fixed** — Webhook `invoice.created` / `invoice.updated` / `quote.created` bodies were empty (built before the invoice had saved its data); assembled at the end of the request now, with real quote status, customer and dates.
...
- **Fixed** — **Template Builder** element colours accept `transparent` and can be cleared (the colour picker forced black), items tables show item names saved by the builder, dates follow the site's date format, the logo element falls back to the business name, and the total block prints credit notes, Paid and Balance due.
- **Fixed** — **Exports were mostly empty**: Bulk Operations and the Export page read meta a saved invoice never has, so amounts, clients, statuses and dates came out blank and the filters matched nothing; they now export the documents' real figures with working client, status and issue-date filters (`easy_invoice_pro_export_field_value` to add columns).
- **Fixed** — The **Client Portal**'s Download and Pay actions did nothing (they linked to handlers that no longer existed): Download streams the server PDF and Pay opens the invoice with the payment panel.
- **Fixed** — **Privacy Tools** exported nothing for a client and the eraser needed a WordPress user; the personal-data exporter now includes the client record, invoices, payments and quotes (matched by email as well as user), and erasure works for guest customers.
- **Fixed** — **Webhook and accounting-sync payloads** carried zero totals for normally saved invoices (they read a stored total that only credit notes and shop orders have); Xero, FreshBooks and QuickBooks payments, Paystack references and webhook bodies now use the documents' real totals, and invoice webhooks carry `amount_due`. `payment.recorded` fires for partial payments as well.
- **Changed** — The Pro zip now unpacks into an `easy-invoice-pro/` folder, so uploading it through Plugins → Add New installs cleanly.
